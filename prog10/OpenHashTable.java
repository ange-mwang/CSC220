...
public class OpenHashTable<K, V> extends AbstractMap<K, V> {
	...
	private int size;
	private int nonNull; //number of non-null entries in the table: either an Entry or DELETED.
	...
	private int find (Object key) {
		...
		// wtf is the key? the name wf is the value?  
		while(table[index] != null && (table[index] == DELETED || !table[index].key.equals(key)) ) { 

			//first time we've seen a traffic cone
			if(table[index] == DELETED && firstCone == -1) { 
				firstCone = index; 
				//index++; 
			}

			index = (index + 1) % table.length; //incrementing

		}
		//return index of node with key if it exists or index of the first null
	...
	private void rehash (int newCapacity) {
		...
		//2 for loops, one for the outside, and one for the linked lists inside
		for(int i = 0; i < oldTable.length; i++) {   //TRAVERSING THE OUTSIDE ARRAY
	...
	private class EntryIterator implements Iterator<Map.Entry<K, V>> {
		...
		public Map.Entry<K, V> next () { //returns the next entry 
			if (!hasNext())
			...
			int next = index; 
			index++; 
			//LOOKING AT THE INDEX AFTER; IF THERE IS NO ONE THERE OR A CONE LOOK AT THE ONE AFTER  
			while(index < table.length && (table[index] == DELETED || table[index] == null )) { 
				index++;
			}
				
				 
			return table[next]; //why teh fuck are we returning next when we didn't do shit to it just index 
		}
