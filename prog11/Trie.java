...
public class Trie <V>
  ...
  private Entry<V> find (String key, int iKey, List<Entry<V>> list) {
    ...
    while(iEntry < list.size() &&  list.get(iEntry).sub.charAt(0) < key.charAt(iKey)) { // Set iEntry to the index of the entry in list
    	//shouldn't we say iKey = 0 ? 
    	iEntry++;
    }
    ...
    ///
    	...
    	//while(key.charAt(iKey) == entry.sub.charAt(iSub) && iKey < key.length() && iSub < entry.sub.length() );
    	//	WTF WHY DOES THE ORDER HERE MATTER??????? THEY'RE ALL && 
  ...
  private V put (String key, int iKey, V value, List<Entry<V>> list) {  //put an entry in. must follow the rules 
    // EXERCISE:
    int iEntry = 0;
    // Set iEntry to the index of the entry in list such that the first
    // character (charAt(0)) of its sub (NOT key) is the same as the
    ...
    if( iEntry == list.size() || list.get(iEntry).sub.charAt(0) > key.charAt(iKey)) { //where it should be is the index BEFORE it 
    	    	// a b  d     key = c  goes before d 
    	
    	//iEntry = list.indexOf(iEntry);
    	Entry<V> entry = new Entry<V>(key.substring(iKey), key, value);
    ...
    //why is this one not -1? 
    if(iSub < entry.sub.length()) { 
    	
    	Entry<V> NewEntry = new Entry(entry.sub.substring(iSub), entry.key, entry.value, entry.list); //UNMATCHED PART OF ENTRY.SUB 
    	

    	entry.list = new ArrayList<Entry<V>>();
    	entry.sub = entry.sub.substring(0,iSub); //MATCHED PART OF ENTRY.SUB
    ...
    //if( iKey > entry.key.length()) { 
    if( iKey == key.length()) { 	
    	if(entry.key == null) { 
    		entry.key = key; 		//set this to what you're trying to put it in
    		entry.value = value; 		
    		size++; 
    		return null; 
    	}
  ...
  private class EntryIterator implements Iterator<Map.Entry<String, V>> {
    ...
    public boolean hasNext () {
      ...
      ///
    	...
    	//while(!stack.peek().hasNext() && stack.isEmpty()== false) { 
  ...
  private String toString (List<Entry<V>> list, int indent) {
    ...
    ///
    String s = "";
    for (Entry<V> entry : list) { //for every entry in the list 
